...
namespace LinqToTwitterXUnitTests.ListTests
{
    class ListTests
    {
        ...
        [Fact]
        public void Create_Does_Not_Populate_ListID()
        {
        ...
        [Fact]
        public void Create_Populates_ListIDResult()
        {
            XElement list = XElement.Parse(this.listXml).Element("lists").Elements("list").First();
            XElement cursor = XElement.Parse(this.listXml);
        ...
        [Fact]
        public void Create_Does_Not_Populate_Slug()
        {
            XElement list = XElement.Parse(this.listXml).Element("lists").Elements("list").First();
            XElement cursor = XElement.Parse(this.listXml);
        ...
        [Fact]
        public void Create_Populates_SlugResult()
        {
            XElement list = XElement.Parse(this.listXml).Element("lists").Elements("list").First();
            XElement cursor = XElement.Parse(this.listXml);
        ...
        readonly string listXml = @"<lists_list>
   <lists type=""array"">
     <list>
       <id>3897042</id>

...
{
    /// <summary>
    ///This is a test class for RelatedResultsTest and is intended
    ///to contain all RelatedResultsTest Unit Tests
    ///</summary>
    [TestClass()]
...
<root type=""array"">
  <item type=""object"">
    ...
    <score type=""number"">1.0</score>
        ...
        ///</summary>
        [TestMethod()]
        public void CreateRelatedResults_Parses_Xml()
        {
            XElement element = XElement.Parse(this.showResultsXml);
            XElement firstElement = element.Element("item").Element("results").Elements("item").First();
                
            var result = RelatedResults.CreateRelatedResults(firstElement);

            Assert.AreEqual("Fork", result.ResultAnnotations.Elements["ConversationRole"]);
            Assert.AreEqual("@elylucas grats!", result.Text);
            Assert.AreEqual(false, result.Truncated);
            Assert.AreEqual(null, result.Place);
            Assert.AreEqual(null, result.Coordinates);
            Assert.AreEqual(false, result.Favorited);
            Assert.AreEqual("64470111079251969", result.IDString);
            Assert.AreEqual(null, result.ValueAnnotations);
            Assert.AreEqual(0, result.RetweetCount);
            Assert.AreEqual(@"<a href=""http://www.tweetdeck.com"" rel=""nofollow"">TweetDeck</a>", result.Source);
            Assert.AreEqual(new DateTime(2011, 4, 30, 23, 24, 06), result.CreatedAt);
            Assert.AreEqual(null, result.Geo);
            Assert.AreEqual("elylucas", result.InReplyToScreenName);
            Assert.AreEqual("64464589072498689", result.InReplyToStatusIDString);
            Assert.AreEqual(null, result.Contributors);
            Assert.AreEqual(false, result.Retweeted);
            Assert.AreEqual(64464589072498689ul, result.InReplyToStatusID);
            Assert.AreEqual("17825699", result.InReplyToUserIDString);
            Assert.AreEqual(17825699ul, result.InReplyToUserID);
            Assert.IsNotNull(result.User);
            Assert.AreEqual(1.0, result.Score);
            Assert.AreEqual("Tweet", result.Kind);
        }
    }
}
